package com.petegg.netty.dispatcher;
...
import com.petegg.netty.domain.MessageQueue;
import com.petegg.netty.handler.GameHandler;
import com.petegg.netty.utils.HttpUtils;
 ...
 * </p>
 * <p>
 * Description: com.petegg.netty.dispatcher
 * </p>
 * <p>
  ...
  public void addMessage(GameRequest request) {
    try {
      MessageQueue messageQueue =
          (MessageQueue) this.sessionMsgQ.get(Integer.valueOf(request.getChannel().hashCode()));
  ...
  private final class MessageWorker implements Runnable {
    ...
    private void handMessageQueue() {
      ...
      GameResponse response =
          new GameResponse(this.request.getChannel(), this.request.getCommand(),
              this.request.getRequestType());
      GameHandler handler =
          (GameHandler) HandlerDispatcher.this.handlerMap.get(Integer.valueOf(messageId));
      if (handler != null)
      ...
      switch (request.getRequestType()) {
        case HTTP:
          HttpUtils.sendHttpResponse(this.request.getCtx(),
              (FullHttpRequest) this.request.getMsg(), response.getResp());
          break;
        case WEBSOCKET_TEXT:
          this.request.getCtx().channel()
              .write(new TextWebSocketFrame(response.getWebSocketRespone()));
          break;
        case SOCKET:
